...
<!-- 
  The fourth script to run in the toolchain. The script transforms the merged treebank file into PML format. The custom attributes were added to 
  the aldt2pml-unicode.xsl stylesheet originally modified from the standard PDT stylesheet by Francesco Mambrini (Harvard University, USA).
-->
   ...
   <xsl:for-each select="$words">
     ...
     <xsl:element name="LM" namespace="http://ufal.mff.cuni.cz/pdt/pml/">
       ...
       <xsl:choose>
         ...
         <!-- if not Greek -->
         <xsl:otherwise>
           <!-- just copy form, lemma and extra annotation -->
           <xsl:copy-of select="@form|@lemma|@seg|@status|@notes"/>
         </xsl:otherwise>
       </xsl:choose>
